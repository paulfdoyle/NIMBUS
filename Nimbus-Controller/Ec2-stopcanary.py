...
if INSTTYPE == 't1.micro':
	try:
		print "stopping the small canary instances now "
		instance = conn.get_all_instances(instance_ids=['i-ae93e6e1'])
		instance[0].instances[0].stop()
	except:
		print "failed to stop the micro canary"
else:
	try:
		print "stopping the large canary instances now "
		instance = conn.get_all_instances(instance_ids=['i-9e13abd2'])
		instance[0].instances[0].stop()
	except:
		print "failed to stop the large canary"
